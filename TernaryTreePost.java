...
public class TernaryTreePost<T> implements Iterable<T> {
    ...
    public class TernaryTreeIterator<T> implements Iterator<T> {
        ...
        private void pushPostOrder(TreeNode<T> node) {
            if (node == null) {
                return;
            }
            ...
            if (node.getRight() != null) {
                pushPostOrder(node.getRight());
            }

            if (node.getMiddle() != null) {
                pushPostOrder(node.getMiddle());
            }

            if (node.getLeft() != null) {
                pushPostOrder(node.getLeft());
            }
        }
    ...
    public static void main(String[] args) {
        TernaryTreePost<Integer> tree = new TernaryTreePost<>();
        tree.insert(5);
        tree.insert(3);
        tree.insert(8);
        tree.insert(2);
